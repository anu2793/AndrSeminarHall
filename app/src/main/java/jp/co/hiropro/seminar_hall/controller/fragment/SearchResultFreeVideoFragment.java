...
public class SearchResultFreeVideoFragment extends FragmentBase implements Paginate.Callbacks {
    ...
    private int page = 1;
    private int mType = AppConstants.TYPE_SEARCH.FAVORITE_CONTENT;
    private BroadcastReceiver mReceive;
    private DividerItemDecoration mCateDivider;

    public SearchResultFreeVideoFragment() {
    }

    public static SearchResultFreeVideoFragment newInstance(String keySearch) {
        SearchResultFreeVideoFragment fragment = new SearchResultFreeVideoFragment();
    ...
    public void initSearchResult() {
        ...
        if (mCateDivider == null) {
            mCateDivider = new DividerItemDecoration(getContext(), DividerItemDecoration.VERTICAL);
            mCateDivider.setDrawable(ContextCompat.getDrawable(getContext(), R.drawable.line_divider_gray));
            lvSearchResult.addItemDecoration(mCateDivider);
        }
...
//            if (requestCode == 103) {
//                VideoDetail videoDetail = data.getParcelableExtra("item");
//                for (int i = 0; i < searchResultList.size(); i++) {
//                    SearchResult obj = searchResultList.get(i);
//                    if (obj.getId() == videoDetail.getId() && obj.getType() == 0) {
//                        obj.getSearchVideo().setIsBought(videoDetail.getHasBoughtVideo());
//                        searchResultList.set(i, obj);
